...
TEST(TestUnsort_Table, check_initialize_str) {
	Tlist<TPair<std::string, std::string>> list;
	for (size_t i = 65; i < 70; i++) {
		char s = i; char s1 = i + 1;
		TPair<std::string, std::string> t(std::string(1, s), std::string(1, s1));
		list.push_back(t);
	}
	Unsorted_Table<std::string, std::string> table(list);
	ASSERT_EQ(table["A"], "B");
	ASSERT_EQ(table["B"], "C");
	/*table.insert("abc");*/
